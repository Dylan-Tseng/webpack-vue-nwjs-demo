...
import {AppID,BankList} from "../config/Const";
...
export default {
    ...
    removeHistoryPage(url){
        let navs = JSON.parse(sessionStorage.getItem("VUE_NAVIGATION"));
        for(let i= navs.length-1;i>=0;i--){
            if(navs[i].indexOf(url)>=0){
                navs.splice(i,1);
            }
        }
        sessionStorage.setItem("VUE_NAVIGATION",JSON.stringify(navs));
    },
    pwdValidate(pwd){
        return /^(?![0-9]+$)(?![a-zA-Z]+$)[0-9A-Za-z_]{8,16}$/.test(pwd);
    },
    getBankIcon(bankName){
        for(let item of BankList){
            if(item.val.indexOf(bankName) >= 0 ){
                return item.icon;
            }
        }
    },
